...
class Chromatogram:
    ...
    def p(self, approx_t):
        eps = 1e6
        j = -1
        for i, t in self.peaks['t_max'].items():
            if np.abs(approx_t - t) < eps:
                eps = np.abs(approx_t - t)
                j = i

        peak = Peak(
                self.peaks['t_start'][j], 
                self.peaks['t_end'][j],
                self.peaks['t_max'][j], 
                self.peaks['area'][j],
                self.peaks['s_max'][j])
...
class Peak:
    def __init__(self, t_start, t_end, tr, area, height):
